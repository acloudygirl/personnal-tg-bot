<#
  功能：停止 bot 守护进程。
  作用：按 PID 停止 watchdog 并清理 PID 文件。
#>
...
$pidPath = Join-Path (Join-Path $projectRoot "logs") "bot_watchdog.pid"
...
function Stop-ProcessSafe {
    ...
    try {
        ...
        Write-Output "Stopped bot watchdog PID=$ProcessId"
        return $true
    } catch {
        Write-Warning "Failed to stop bot watchdog PID=$($ProcessId): $($_.Exception.Message)"
        return $false
    }
}
...
if ($all.Count -eq 0) {
    Write-Output "Bot watchdog is not running."
} else {
...
Get-ChildItem -Path $logDir -Filter "bot_watchdog*.pid" -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
